...
export default {
	...
	methods: {
		toggleInfoWindow: function(r) {
			...
			this.$bus.$emit("showDetailedResult",r.place_id);
		},
	...
	mounted: function() {
		...
		this._showDetailedResult = (placeId) => {
			this.$bus.$emit("getDetailedResult")
		}
		this.$bus.$on("clearSearchResults",this._clearSearchResults);
		this.$bus.$on("showDetailedResult",this._showDetailedResult);
	},
